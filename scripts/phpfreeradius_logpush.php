...
{
	$GLOBALS["config"]["lock_file"] = "/var/lock/phpfreeradius_lock_logpush";
}
else
{
		...
		catch (SoapFault $exception)
		{
					...
					// disruption will not lead to this occuring, since an ACCESS_DENIED response is required.
                        ...
                        elseif ($exception->getMessage() == "Could not connect to host")
                        {
                                ...
                                log_write("error", "script", "Unable to connect to the host, probably apache/phpfreeradius outage or server unreachable");
