...
if (user_online())
{
	...
	$_SESSION["error"]["username_phpfreeradius"] = "error";
	$_SESSION["error"]["password_phpfreeradius"] = "error";
...
else
{
	// check & convert input
	$instance	= NULL;
	$username	= security_form_input("/^[A-Za-z0-9.]*$/", "username_phpfreeradius", 1, "Please enter a username.");
	$password	= security_form_input("/^\S*$/", "password_phpfreeradius", 1, "Please enter a password.");
	...
	else
	{
		...
		if (!$_SESSION["error"]["message"])
		{
			...
			$_SESSION["error"]["username_phpfreeradius-error"] = 1;
			$_SESSION["error"]["password_phpfreeradius-error"] = 1;
		}

		if ($result == -3)
		{
			$_SESSION["error"]["instance_phpfreeradius-error"] = 1;
		}
